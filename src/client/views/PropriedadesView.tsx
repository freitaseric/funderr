...
import { AlertCircle, ExternalLink, Home, MapPin, Plus, Search } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {canEdit && (
          <button onClick={openNewModal} className="bg-[#1351b4] hover:bg-blue-700 text-white font-bold px-4 py-2.5 rounded-lg text-sm flex items-center gap-2">
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 flex flex-col md:flex-row items-center gap-3">
        <div className="flex items-center gap-2 flex-1 w-full">
          ...
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por propriedade, município ou beneficiário" className="w-full text-sm outline-hidden" />
        </div>
        <select value={selectedMunicipio} onChange={(event) => setSelectedMunicipio(event.target.value)} className="w-full md:w-64 px-3 py-2 border rounded-md text-xs bg-slate-50">
              ...
              {filtered.length === 0 ? (
                ...
                <tr key={property.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-3.5 text-right">
                    {canEdit && <button onClick={() => openEditModal(property)} className="text-xs font-bold text-blue-700 bg-blue-50 px-2.5 py-1 rounded border border-blue-200">Editar</button>}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
            <Field label="Beneficiário responsável *" wide>
              <select required disabled={Boolean(selectedProperty)} value={formData.beneficiaryId} onChange={(event) => setFormData({ ...formData, beneficiaryId: event.target.value })} className="input">
            ...
            <Field label="Denominação do imóvel *" wide><input required minLength={2} value={formData.denominacao} onChange={(event) => setFormData({ ...formData, denominacao: event.target.value })} className="input" /></Field>
            <Field label="Localização / roteiro de acesso" wide><textarea rows={2} value={formData.endereco} onChange={(event) => setFormData({ ...formData, endereco: event.target.value })} className="input" /></Field>
            <Field label="Município"><select value={formData.municipio} onChange={(event) => setFormData({ ...formData, municipio: event.target.value as RoraimaMunicipality | "" })} className="input"><option value="">Selecione</option>{RORAIMA_MUNICIPALITIES.map((municipio) => <option key={municipio}>{municipio}</option>)}</select></Field>
            <Field label="Forma de ocupação"><select value={formData.formaOcupacao} onChange={(event) => setFormData({ ...formData, formaOcupacao: event.target.value })} className="input"><option value="">Selecione</option><option>Proprietário</option><option>Posseiro</option><option>Arrendatário</option><option>Meeiro</option><option>Concessionário de uso</option><option>Comodatário</option></select></Field>
            <Field label="Área total (ha)"><input type="number" min="0" step="0.01" value={formData.areaTotal || ""} onChange={(event) => setFormData({ ...formData, areaTotal: Number(event.target.value) })} className="input" /></Field>
            <Field label="Área disponível (ha)"><input type="number" min="0" step="0.01" value={formData.areaDisponivel} onChange={(event) => setOptionalNumber("areaDisponivel", event.target.value)} className="input" /></Field>
            <Field label="Área legal/reserva (ha)"><input type="number" min="0" step="0.01" value={formData.areaLegal} onChange={(event) => setOptionalNumber("areaLegal", event.target.value)} className="input" /></Field>
            <Field label="Tempo de exploração"><input value={formData.tempoExploracao} onChange={(event) => setFormData({ ...formData, tempoExploracao: event.target.value })} placeholder="Ex.: 8 anos" className="input" /></Field>
            <Field label="Módulo fiscal / rural"><input value={formData.modulo} onChange={(event) => setFormData({ ...formData, modulo: event.target.value })} className="input" /></Field>
            <Field label="Documento fundiário existente" wide><input value={formData.documentoExistente} onChange={(event) => setFormData({ ...formData, documentoExistente: event.target.value })} placeholder="Tipo, número, matrícula ou identificação do documento" className="input" /></Field>
            <Field label="Latitude"><input type="number" step="any" value={formData.latitude ?? ""} onChange={(event) => setFormData({ ...formData, latitude: event.target.value === "" ? null : Number(event.target.value) })} className="input font-mono" /></Field>
            <Field label="Longitude"><input type="number" step="any" value={formData.longitude ?? ""} onChange={(event) => setFormData({ ...formData, longitude: event.target.value === "" ? null : Number(event.target.value) })} className="input font-mono" /></Field>
            <Field label="Confrontação norte"><input value={formData.confrontacaoNorte} onChange={(event) => setFormData({ ...formData, confrontacaoNorte: event.target.value })} className="input" /></Field>
            <Field label="Confrontação sul"><input value={formData.confrontacaoSul} onChange={(event) => setFormData({ ...formData, confrontacaoSul: event.target.value })} className="input" /></Field>
            <Field label="Confrontação leste"><input value={formData.confrontacaoLeste} onChange={(event) => setFormData({ ...formData, confrontacaoLeste: event.target.value })} className="input" /></Field>
            <Field label="Confrontação oeste"><input value={formData.confrontacaoOeste} onChange={(event) => setFormData({ ...formData, confrontacaoOeste: event.target.value })} className="input" /></Field>
            <Field label="Administração / responsável local" wide><textarea rows={2} value={formData.administracao} onChange={(event) => setFormData({ ...formData, administracao: event.target.value })} className="input" /></Field>
          </div>

          <div className="flex justify-end gap-3 pt-4 border-t">
            <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 border rounded-lg text-xs font-semibold">Cancelar</button>
            <button type="submit" disabled={saving} className="px-5 py-2 bg-[#1351b4] text-white font-bold rounded-lg text-xs disabled:opacity-50">{saving ? "Salvando..." : "Salvar rascunho"}</button>
          </div>
        </form>
      </Modal>
    </div>

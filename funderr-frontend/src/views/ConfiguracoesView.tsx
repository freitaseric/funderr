...
import { Check, Shield, Sparkles, ToggleLeft, ToggleRight } from "../components/ui/icons";
...
export const ConfiguracoesView: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="bg-surface p-6 rounded-2xl border border-slate-200 space-y-4">
        <div className="flex items-center gap-2 border-b pb-3">
          <Sparkles className="w-5 h-5 text-[#386a20]" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(Object.keys(flagDescriptions) as (keyof RemoteConfigFlags)[]).map((key) => (
              <div key={key} className="p-4 rounded-xl border flex items-center justify-between gap-4">
                ...
                <AppButton disabled={!isAdmin || saving} onClick={() => void handleToggleFlag(key)} className="disabled:opacity-40" aria-label={`Alternar ${flagDescriptions[key].title}`}>
      ...
      {isAdmin && (
        <section className="bg-surface p-6 rounded-2xl border border-slate-200 space-y-5">
          <div className="flex items-center justify-between border-b pb-3">
            <div className="flex items-center gap-2"><Shield className="w-5 h-5 text-[#386a20]" /><h3 className="text-base font-bold">Usuários e perfis</h3></div>
              ...
              <tbody className="divide-y">
                {users.map((account) => (
                  <tr key={account.id}>
                    ...
                    <td className="px-4 py-2.5"><AppSelect disabled={saving} value={account.role || ""} onChange={(event) => void handleUpdateUser(account, { role: event.target.value as UserRole })} className="px-2 py-1 border rounded">{account.role === null && <option value="">Sem papel</option>}{assignableRoles.map((role) => <option key={role}>{role}</option>)}</AppSelect></td>
                    <td className="px-4 py-2.5"><AppSelect disabled={saving} value={account.status} onChange={(event) => void handleUpdateUser(account, { status: event.target.value as UserStatus })} className="px-2 py-1 border rounded"><option value="ACTIVE">ACTIVE</option><option value="PENDING">PENDING</option><option value="DISABLED">DISABLED</option></AppSelect></td>
                  </tr>
                ))}
              </tbody>
            </table>
